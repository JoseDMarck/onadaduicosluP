...
if( ! empty($row_type) && $row_type == 'parallax'){
	...
	if(discussion_options()->getOptionValue('parallax_on_off') == 'off'){
...
if($row_type == 'row' && $css_animation != ''){
	...
	if($transition_delay !== ''){
		...
		$inner_start .= '<div '.discussion_get_inline_style($animation_styles).'>';
		$inner_end .= '</div>';
	}else{
		$inner_start .= '<div>';
		$inner_end .= '</div>';
...
if($video == 'show_video'){
	...
	if($video_image) {
		$video_output .= '<div class="mkd-mobile-video-image" ' . discussion_get_inline_attr($video_mobile_style, 'style') . ')"></div>';
	}
	
	$video_output .= '<div ' . discussion_get_class_attribute($video_overlay_class) . discussion_get_inline_attr($video_overlay_style, 'style') . '></div>';
	$video_output .= '<div class="mkd-video-wrap">';
	$video_output .= '<video class="mkd-video" width="1920" height="800" '. discussion_get_inline_attr($video_attrs, 'poster')  .' controls="controls" preload="auto" loop autoplay muted>';
	...
	if($v_image) {
		$video_output .= '<img ' . discussion_get_inline_attr($v_image, 'src') . ' width="1920" height="800" title="No video playback capabilities" alt="<?php esc_html__("Video thumb","discussionwp"); ?>" />';
	}
	$video_output .='</object>';
	$video_output .='</video>';
	$video_output .='</div>';
