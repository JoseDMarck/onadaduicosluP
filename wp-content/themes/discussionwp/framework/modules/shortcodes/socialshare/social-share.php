<?php
namespace Discussion\Modules\SocialShare;
...
class SocialShare implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		vc_map(array(
			'name' => esc_html__('Mikado Social Share', 'discussionwp'),
			'base' => $this->getBase(),
	...
	public function render($atts, $content = null) {
		...
		$params['enable_social_share'] = (discussion_options()->getOptionValue('enable_social_share') == 'yes') ? true : false;

		//Is social share enabled for post type
		$post_type = get_post_type();
		$params['enabled'] = (discussion_options()->getOptionValue('enable_social_share_on_'.$post_type) == 'yes') ? true : false;
		...
		if ($params['enable_social_share']) {
			if ($params['enabled']) {
				$html .= discussion_get_shortcode_module_template_part('templates/' . $params['type'], 'socialshare', '', $params);
			}
		}
	...
	private function getSocialNetworksParams($params) {
		...
		foreach ($this->socialNetworks as $net) {
			...
			$html = '';
			if (discussion_options()->getOptionValue('enable_'.$net.'_share') == 'yes') {
				...
				$params['custom_icon'] = (discussion_options()->getOptionValue($net.'_icon')) ? discussion_options()->getOptionValue($net.'_icon') : '';
				$html = discussion_get_shortcode_module_template_part('templates/parts/network', 'socialshare', '', $params);
			}
		...
		switch ($net) {
            case 'facebook':
                if(wp_is_mobile()) {
                    ...
                    $link = 'window.open(\'http://www.facebook.com/sharer.php?s=100&amp;p[title]=' . urlencode(discussion_addslashes(get_the_title())) . '&amp;p[url]=' . urlencode(get_permalink()) . '&amp;p[images][0]=' . $image[0] . '&amp;p[summary]=' . urlencode(discussion_addslashes(get_the_excerpt())) . '\', \'sharer\', \'toolbar=0,status=0,width=620,height=280\');';
                }
                break;
            case 'twitter':
                ...
                if(wp_is_mobile()) {
                    $link = 'window.open(\'https://twitter.com/intent/tweet?text=' . urlencode(discussion_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                } else {
                    $link = 'window.open(\'http://twitter.com/home?status=' . urlencode(discussion_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                }
                break;
			case 'google_plus':
