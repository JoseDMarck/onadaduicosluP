<?php
namespace Discussion\Modules\Header\Lib;
/**
...
class HeaderConnector {
    ...
    public function connect($config = array()) {
        do_action('discussion_pre_header_connect');
        ...
        if(!empty($config['affect_content'])) {
            add_filter('discussion_content_elem_style_attr', array($this, 'contentMarginFilter'));
        }

        if(!empty($config['affect_title'])) {
            add_filter('discussion_title_content_padding', array($this, 'titlePaddingFilter'));
        }

        do_action('discussion_after_header_connect');
    }
     ...
     *
     * Hooks to discussion_title_content_padding filter
     *
     * @return int
     */
