...
include 'mkd-instagram-helper.php';

/**
 * Class MikadoInstagramApi
 */
class MikadoInstagramApi {
    /**
    ...
    private $apiURL;
    /**
     * @var MikadoInstagramApi instance of current class
     */
    private static $instance;

    /**
     * Helper object
     * @var MikadoInstagramHelper
     */
    private $helper;

    const CODE_OPTION_NAME = 'mkd_instagram_code';
    const USER_ID_OPTION_NAME = 'mkd_instagram_user_id';
    const ACCESS_TOKEN_OPTION_NAME = 'mkd_instagram_access_token';
    const CODE_REDIRECT_URI_OPTION_NAME = 'mkd_instagram_code_redirect_uri';
    const INST_USER_SCREEN_NAME_FIELD = 'mkd_instagram_screen_name';
    ...
    private function __construct() {
        $this->clientID = '9e2ab90b6764468583333d424defe4d5';
        $this->clientSecret = '204f0547202041b4bd020bec25bdd9d7';
        ...
        $this->helper = new MikadoInstagramHelper();
    }
    ...
    public function getScreenName() {
        return get_option(self::INST_USER_SCREEN_NAME_FIELD);
    }
     ...
     * @return string
     *
     * @see MikadoInstagramApi::buildCurrentPageURI()
     */
     ...
     *
     * @see MikadoInstagramApi::fetchData()
     */
     ...
     * @return stdClass
     *
     * @see MikadoInstagramApi::getAccessToken()
     */
    ...
    public function getAccessToken() {
        ...
        if(empty($this->code)) {
            $returnObject->status = 'error';
            $returnObject->message = 'User hasn\'t connected with Instagram';
            ...
            if(is_wp_error($httpResponse) || (isset($httpResponse['response']['code']) && $httpResponse['response']['code'] !== 200)) {
                $returnObject->status = 'error';
                $returnObject->message = 'Can\'t connect with Instagram API';
            ...
            if(isset($responseBody['access_token']) && isset($responseBody['user']['id'])) {
				...
				//get user data
				$userDataResponse = wp_remote_get($this->apiURL.'/users/'.$this->userID.'/?access_token='.$this->accessToken);
				$httpUserData = json_decode($userDataResponse['body'],true);

				//store username
				if (isset($httpUserData['data']['username'])){
					update_option(self::INST_USER_SCREEN_NAME_FIELD, $httpUserData['data']['username']);
				}

            } else {
                $returnObject->status = 'error';
                $returnObject->message = 'Can\'t get Instagram access token';
