...
class Mikado_Import {
    ...
    function __construct(){
        ...
        add_action('admin_init', array(&$this, 'discussion_register_theme_settings'));
    ...
    public function import_options($file){
        $options = $this->file_options($file);
        update_option( 'mkd_options_discussion', $options);
    ...
    function mkd_admin_import() {
        if(mkd_core_theme_installed()) {
			...
			$this->pagehook = add_submenu_page(
				...
				array($discussion_Framework->getSkin(), 'renderImport')
			);
	
	        add_action('admin_print_scripts-'.$this->pagehook, 'discussion_enqueue_admin_scripts');
	        add_action('admin_print_styles-'.$this->pagehook, 'discussion_enqueue_admin_styles');
        }
    }
...
function mkd_init_import_object(){
	global $discussion_import_object;
	$discussion_import_object = new Mikado_Import();
}
...
if(!function_exists('mkd_dataImport')){
    function mkd_dataImport(){
        ...
        if ($_POST['import_attachments'] == 1)
            $discussion_import_object->attachments = true;
        else
            $discussion_import_object->attachments = false;

        $folder = "discussion/";
        if (!empty($_POST['example']))
            $folder = $_POST['example']."/";

        $discussion_import_object->import_content($folder.$_POST['xml']);
...
if(!function_exists('mkd_widgetsImport')){
    function mkd_widgetsImport(){
        ...
        $discussion_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
...
if(!function_exists('mkd_optionsImport')){
    function mkd_optionsImport(){
        ...
        $discussion_import_object->import_options($folder.'options.txt');
...
if(!function_exists('mkd_otherImport')){
    function mkd_otherImport(){
        ...
        $discussion_import_object->import_options($folder.'options.txt');
        $discussion_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
        $discussion_import_object->import_menus($folder.'menus.txt');
        $discussion_import_object->import_settings_pages($folder.'settingpages.txt');
